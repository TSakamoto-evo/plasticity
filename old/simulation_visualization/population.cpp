...
void Population::return_ind(std::vector<double>& ret_genotypes,
  ...
  std::vector<int> dist_num;
  for(const auto& i: pop){
    dist_num.push_back(i.return_size());
  }

  std::discrete_distribution<> dist(dist_num.begin(), dist_num.end());

  int index = dist(mt);
  pop.at(index).return_genotype(ret_genotypes);
  pop.at(index).return_interactions(ret_interactions);
  pop.at(index).return_env_effects(ret_env_effects);

  pop.at(index).development(para.alpha, para.dev_period, s1_env);
  pop.at(index).return_phenotype(ret_adult1);

  pop.at(index).development(para.alpha, para.dev_period, s2_env);
  pop.at(index).return_phenotype(ret_adult2);
}

void Population::return_dev_process(const double s1_env, const double s2_env,
  ...
  int index = dist(mt);
  pop.at(index).ret_dev_process(para.alpha, para.dev_period, s1_env, ret_dev1);
  pop.at(index).ret_dev_process(para.alpha, para.dev_period, s2_env, ret_dev2);
}

std::vector<std::vector<double>> Population::random_genotype(const int trial,
  const double s1_env, const double s2_env){
  ...
  std::discrete_distribution<> dist(dist_num.begin(), dist_num.end());
  std::uniform_real_distribution<> det_g(0.0, para.max_g);
  ...
  for(int i = 0; i < trial; i++){
    ...
    for(int j = 0; j < para.num_g; j++){
      tmp_g.push_back(det_g(mt));
    }

    std::vector<double> p1, p2;
    p1 = pop.at(index).ret_dev_random(para.alpha, para.dev_period, s1_env, tmp_g);
    p2 = pop.at(index).ret_dev_random(para.alpha, para.dev_period, s2_env, tmp_g);

    tmp_g.insert(tmp_g.end(), p1.begin(), p1.end());
    tmp_g.insert(tmp_g.end(), p2.begin(), p2.end());
...
std::vector<std::vector<double>> Population::grad_env(
  const std::vector<double>& grad_cues){
  ...
  for(const auto& i: grad_cues){
    std::vector<double> tmp_p;
    pop.at(index).development(para.alpha, para.dev_period, i);
    pop.at(index).return_phenotype(tmp_p);
    ret.push_back(tmp_p);
  }
