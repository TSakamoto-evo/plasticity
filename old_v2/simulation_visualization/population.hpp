...
private:
  ...
  std::vector<std::vector<int>> ind_index_list;
  std::vector<std::vector<double>> ind_effect_list;
  std::vector<long long int> ind_geno_index;

  std::vector<int> ind_rand_ini;

  std::vector<std::uniform_int_distribution<>> choose_inds;

public:
  Population(const Parameters input_para);
  void development(const double env_cue, const std::vector<double> s_vec);
  void selection();
  long long int return_total_geno() const;
  void gen_mutation();
  void one_generation(const double env_cue, const std::vector<double> s_vec,
    const int threshold);
  void selection_many_segregation_v2();
  ...
  void check_env_grad(const std::vector<double>& s1_vec,
    ...
    std::vector<int>& ret);
  void make_output_file(const int var_run_index, const int para_index,
    const int time, const std::vector<double> env_cues,
    const std::vector<std::vector<double>> s_vecs,
