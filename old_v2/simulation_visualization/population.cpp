...
void Population::regi_genotype(){
  std::ofstream ofs("regi_genotype.txt", std::ios::app);

  std::vector<double> geno, env;
  std::vector<std::vector<double>> inte;

  for(const auto& i: pop){
    i.return_genotype(geno);
    i.return_env_effects(env);
    i.return_interactions(inte);

    int size = i.return_size();

    ofs << gen << "\t" << size;

    for(const auto& j: geno){
      ofs << "\t" << j;
    }

    for(const auto& j: env){
      ofs << "\t" << j;
    }

    for(const auto& j: inte){
      for(const auto& k: j){
        ofs << "\t" << k;
      }
    }
