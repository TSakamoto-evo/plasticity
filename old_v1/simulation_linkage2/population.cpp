...
void Population::gen_mutation(){
  ...
  for(int i = 0; i < num_mut; i++){
    ...
    if(type == 0){
      ...
      std::vector<double> effect = add.add_mut(locus, d_mu_g(mt), allele_index, para.max_g);
      ...
      std::vector<double> effect = add.add_mut(locus, d_mu_b(mt), allele_index, para.max_g);
      ...
      int allele_index = mutation.return_index(locus1) + 1;
      std::vector<double> effect = add.add_mut(locus, d_mu_c(mt), allele_index, para.max_g);
      mutation.add_mutation(locus1, effect);
      pop.push_back(add);
    }
  }
