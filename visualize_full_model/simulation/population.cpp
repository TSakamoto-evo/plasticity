...
Population::Population(const Parameters input_para,
  ...
  std::ofstream ind_regi("ind_regi.txt");
  ind_regi.close();
}
...
void Population::gen_mutation(){
  ...
  for(int i = 0; i <= max_index; i++){
    ...
    sum += pop[i].ret_ind_num();
  }

  if(gen % 10000 == 0){
    std::cout << gen << "\t" << max_index << "\t" << sum << std::endl;
  }
...
void Population::one_generation(){
  ...
  auto dur1 = time1 - time0;
  auto dur2 = time2 - time1;
  auto dur3 = time3 - time2;

  time_dev += std::chrono::duration_cast<std::chrono::milliseconds>(dur1).count();
  time_sele += std::chrono::duration_cast<std::chrono::milliseconds>(dur2).count();
  time_mut += std::chrono::duration_cast<std::chrono::milliseconds>(dur3).count();

  gen++;

  if(gen % 1000 == 0){
    //std::cout << time_dev << "\t" << time_sele << "\t" << time_mut << "\t" << mode << std::endl;
  }
}
...
void Population::output_ind() const{
  std::ofstream ind_regi("ind_regi.txt", std::ios::app);

  std::vector<double> geno;
  std::vector<std::vector<double>> inte;
  std::vector<double> env;

  pop.at(0).return_genotype(geno);
  pop.at(0).return_interactions(inte);
  pop.at(0).return_env_effects(env);

  ind_regi << gen << "\t" << para.num_g;
